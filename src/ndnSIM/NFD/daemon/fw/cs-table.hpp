...
#ifndef NFD_DAEMON_FW_CS_TABLE_HPP
#define NFD_DAEMON_FW_CS_TABLE_HPP
...
public:
   ...
   */
  void
  add(shared_ptr<Cs> cs,FaceId faceId);
...
public: // enumeration
  typedef std::map<FaceId, shared_ptr<Cs>> CsMap;

  typedef boost::indirected_range<const boost::select_second_const_range<CsMap>> ForwardRange;
...
public: // signals
   ...
   */
  signal::Signal<CsTable, Face&> afterAdd;

  /** \brief fires before a face is removed
   *
   *  When this signal is emitted, face is still in FaceTable and has valid FaceId.
   */
  signal::Signal<CsTable, Face&> beforeRemove;

private:
  void
  addImpl(shared_ptr<Cs> cs, FaceId faceId);
...
#endif // NFD_DAEMON_FW_CS_TABLE_HPP
